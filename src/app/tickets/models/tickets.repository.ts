...
export class TicketRepository {
  ...
  getTickets(isAllTickets = false): Tickets[] {
    if (!isAllTickets) {
      return this.tickets.filter(
        (ticket) => !ticket.status.toLowerCase().startsWith('close')
      );
    }
    return this.tickets;
  }
  ...
  async saveTickets(item: Tickets) {
    ...
    if (item._id == null || item._id == '') {
      this.dataSource.insertTickets(item).subscribe((response) => {
        if (response._id) {
          // If API created
      ...
      this.dataSource.updateTickets(item).subscribe((resp) => {
        ...
        if (response.success == true) {
          ...
          this.tickets.splice(
            this.tickets.findIndex((i) => i._id == item._id),
            1,
            item
          );
        } else {
  ...
  deleteTickets(id: string) {
    this.dataSource.deleteTickets(id).subscribe((response) => {
      if (response.success) {
        this.tickets.splice(
          this.tickets.findIndex((item) => item._id == id),
          1
        );
      } else {
        alert(`Error: ${response.message}`);
